...
const Number uint8 = 208

// Register the algorithm at test-binary init time.
func init() {
	if err := dns.RegisterAlgorithm(Number, New()); err != nil {
		panic("mayo5 test init: RegisterAlgorithm: " + err.Error())
	}
}

func TestRegistered(t *testing.T) {
	if name := dns.AlgorithmToString[Number]; name != "MAYO5" {
		t.Errorf("AlgorithmToString[%d] = %q, want MAYO5", Number, name)
	}
...
func TestPrivateKeyRoundTrip(t *testing.T) {
	...
	if !strings.Contains(s, "Algorithm: 208 (MAYO5)") {
