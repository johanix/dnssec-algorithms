...
func init() {
	if err := dns.RegisterAlgorithm(Number, New()); err != nil {
		panic("falcon512 test init: RegisterAlgorithm: " + err.Error())
	}
}

func TestRegistered(t *testing.T) {
	if name := dns.AlgorithmToString[Number]; name != "FALCON512" {
		t.Errorf("AlgorithmToString[%d] = %q, want FALCON512", Number, name)
	}
...
func TestPrivateKeyRoundTrip(t *testing.T) {
	...
	if !strings.Contains(s, "Algorithm: 201 (FALCON512)") {
