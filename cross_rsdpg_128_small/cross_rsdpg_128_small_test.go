...
const Number uint8 = 214

// Register the algorithm at test-binary init time.
func init() {
	if err := dns.RegisterAlgorithm(Number, New()); err != nil {
		panic("cross_rsdpg_128_small test init: RegisterAlgorithm: " + err.Error())
	}
}

func TestRegistered(t *testing.T) {
	if name := dns.AlgorithmToString[Number]; name != "CROSSRSDPG128SMALL" {
		t.Errorf("AlgorithmToString[%d] = %q, want CROSSRSDPG128SMALL", Number, name)
	}
...
// TestKeyAndSigSizes documents the on-wire sizes that make CROSS
// RSDP-G-128-small a KSK-only candidate (small public key, large
// signature). These are the liboqs 0.15.0 parameter-set values.
func TestKeyAndSigSizes(t *testing.T) {
	const (
		wantPubKey = 54   // OQS_SIG_cross_rsdpg_128_small_length_public_key
		wantSig    = 8960 // OQS_SIG_cross_rsdpg_128_small_length_signature
	)
	if n, err := liboqs.PublicKeySize(AlgName); err != nil {
		t.Fatalf("PublicKeySize: %v", err)
	} else if n != wantPubKey {
		t.Errorf("PublicKeySize = %d, want %d", n, wantPubKey)
	}

	s, err := liboqs.Generate(AlgName)
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}
	sig, err := s.Sign(nil, []byte("size probe"), crypto.Hash(0))
	if err != nil {
		t.Fatalf("Sign: %v", err)
	}
	if len(sig) != wantSig {
		t.Errorf("signature length = %d, want %d", len(sig), wantSig)
	}
}
...
func TestPrivateKeyRoundTrip(t *testing.T) {
	...
	if !strings.Contains(s, "Algorithm: 214 (CROSSRSDPG128SMALL)") {
