...
// oid is a placeholder PKCS#8 algorithm OID for CROSS RSDP-G-128-small.
// NIST additional-signatures candidates have no assigned PKIX OID;
// using the same private tail convention as the other algorithm
// subpackages in this repo (2.16.840.1.101.3.4.3.99.N).
var oid = asn1.ObjectIdentifier{2, 16, 840, 1, 101, 3, 4, 3, 99, 12}
...
func (pkcs8Codec) ParsePKCS8(der []byte) (crypto.PrivateKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cross-rsdpg-128-small private key decode: %w", err)
	}
	return s, nil
}
