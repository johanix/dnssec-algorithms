...
const Number uint8 = 209

// Register the algorithm at test-binary init time.
func init() {
	if err := dns.RegisterAlgorithm(Number, New()); err != nil {
		panic("falcon1024 test init: RegisterAlgorithm: " + err.Error())
	}
}

func TestRegistered(t *testing.T) {
	if name := dns.AlgorithmToString[Number]; name != "FALCON1024" {
		t.Errorf("AlgorithmToString[%d] = %q, want FALCON1024", Number, name)
	}
...
func TestPrivateKeyRoundTrip(t *testing.T) {
	...
	if !strings.Contains(s, "Algorithm: 209 (FALCON1024)") {
