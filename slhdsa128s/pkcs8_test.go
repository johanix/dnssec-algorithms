...
func TestPKCS8RoundTrip(t *testing.T) {
	_, sk, err := slhdsa.GenerateKey(rand.Reader, ParamSet)
	if err != nil {
	...
	if !ok {
		t.Fatalf("pkcs8.Parse returned %T, want *slhdsa.PrivateKey", parsed)
	}
...
func TestPKCS8ParseRejectsWrongOID(t *testing.T) {
	...
	// The OID's last component is 20 (SLH-DSA-SHA2-128s). Flip it to
	// 99 (an unassigned trailing value) and confirm the codec
	// rejects it as ErrUnsupported.
	prefix := []byte{0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x03}
	i := bytes.Index(der, prefix)
	if i < 0 || i+len(prefix) >= len(der) {
		t.Fatalf("could not locate SLH-DSA OID prefix in DER")
	}
	if der[i+len(prefix)] != 20 {
		t.Fatalf("expected OID last component 20, got %d", der[i+len(prefix)])
	}
	der[i+len(prefix)] = 99
