...
const Number uint8 = 211

// Register the algorithm at test-binary init time.
func init() {
	if err := dns.RegisterAlgorithm(Number, New()); err != nil {
		panic("snova25_8_3 test init: RegisterAlgorithm: " + err.Error())
	}
}

func TestRegistered(t *testing.T) {
	if name := dns.AlgorithmToString[Number]; name != "SNOVA25_8_3" {
		t.Errorf("AlgorithmToString[%d] = %q, want SNOVA25_8_3", Number, name)
	}
...
func TestPrivateKeyRoundTrip(t *testing.T) {
	...
	if !strings.Contains(s, "Algorithm: 211 (SNOVA25_8_3)") {
