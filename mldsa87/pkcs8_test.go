...
func TestPKCS8RoundTrip(t *testing.T) {
	...
	if !ok {
		t.Fatalf("pkcs8.Parse returned %T, want *mldsa87.PrivateKey", parsed)
	}
	if !sk.Equal(sk2) {
...
func TestPKCS8ParseRejectsWrongOID(t *testing.T) {
	...
	// inner OID 2.16.840.1.101.3.4.3.99 is not assigned to ML-DSA-87.
	// We synthesize it by editing a real marshaled ML-DSA-87 key.
	_, sk, err := mldsa87.GenerateKey(nil)
	if err != nil {
		t.Fatalf("GenerateKey: %v", err)
	}
	...
	// The OID's last component is 19 in the encoding. Find its
	// position by scanning for the OID prefix bytes and flip the
	// trailing 19 to 99 (both single-byte values).
	...
	if der[i+len(prefix)] != 19 {
		t.Fatalf("expected OID last component 19, got %d", der[i+len(prefix)])
	}
	der[i+len(prefix)] = 99
