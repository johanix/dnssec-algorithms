// Package mayo2 provides a [dns.Algorithm] implementation of MAYO-2
// (NIST onramp post-quantum signature candidate, oil-and-vinegar
// based) for DNSSEC SIG(0) transaction signing and RRSIG zone
// signing.
//
// Algorithm number 206 is used. IANA has not yet assigned a codepoint
// for MAYO in the DNS Security Algorithm Numbers registry.
//
// MAYO-2 is a NIST security level 1 parameter set, like MAYO-1, but
// trades a much larger public key for a much smaller signature:
// signatures are 186 bytes; public keys 4912 bytes; secret keys are a
// tiny 24-byte seed.
//
// # Build requirement
//
...
//
//	import _ "github.com/johanix/dnssec-algorithms/mayo2"
package mayo2
...
// AlgName is the liboqs identifier for MAYO-2.
const AlgName = "MAYO-2"

// Impl is the MAYO-2 [dns.Algorithm] implementation. Construct with
// [New]; pass the returned value to [dns.RegisterAlgorithm].
type Impl struct{}

// New returns a [dns.Algorithm] implementation for MAYO-2.
func New() *Impl { return &Impl{} }

func (*Impl) Name() string      { return "MAYO2" }
...
func (*Impl) PublicKeyFromWire(buf []byte) (crypto.PublicKey, error) {
	...
	if len(buf) != want {
		return nil, fmt.Errorf("mayo-2 public key length %d, want %d", len(buf), want)
	}
...
func (*Impl) PrivateKeyToString(priv crypto.PrivateKey) (string, error) {
	...
	if s.AlgName != AlgName {
		return "", fmt.Errorf("mayo-2 PrivateKeyToString: signer is for %q", s.AlgName)
	}
