...
func init() {
	if err := dns.RegisterAlgorithm(Number, New()); err != nil {
		panic("mldsa65 test init: RegisterAlgorithm: " + err.Error())
	}
}

func TestRegistered(t *testing.T) {
	if name := dns.AlgorithmToString[Number]; name != "MLDSA65" {
		t.Errorf("AlgorithmToString[%d] = %q, want MLDSA65", Number, name)
	}
...
func TestPrivateKeyRoundTrip(t *testing.T) {
	...
	sk, ok := priv.(*mldsa65.PrivateKey)
	if !ok {
		t.Fatalf("Generate returned %T, want *mldsa65.PrivateKey", priv)
	}

	s := keyrr.PrivateKeyString(sk)
	if !strings.Contains(s, "Algorithm: 212 (MLDSA65)") {
	...
	if !ok {
		t.Fatalf("NewPrivateKey returned %T, want *mldsa65.PrivateKey", parsed)
	}
	if !sk.Equal(sk2) {
